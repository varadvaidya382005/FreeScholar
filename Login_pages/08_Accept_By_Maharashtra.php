...
if(isset($_GET['user_name'])) {
    ...
    $check_query = "SELECT * FROM `status` WHERE `user_name` = '$user_id' AND `institute_status` = 'Accepted' AND `institute_status` = 'Accepted'";
    ...
    if(mysqli_num_rows($check_result) > 0) {
        ...
        $insert_query = "UPDATE `status` SET `home_status`='Accepted' WHERE `user_name`='$user_id'";
        $insert_result = mysqli_query($con1, $insert_query);

        if($insert_result) {
            // If insertion is successful, display a success message
            echo "<table border =5><td><b style='color: red;'>$user_id</b>:: Home State  Verification Completed and data is passed to Email Generation</table></td>";
            echo "<a href='mailsend.php?user_name=$user_id'><button>Send Mail</button></a>";
        } else {
